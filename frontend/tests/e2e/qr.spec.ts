...
test.describe("Admin — QR step 5", () => {
  ...
  test("step 5 tab appears in the wizard stepper", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto(`/admin/items/${ITEM_ID}`);

    await expect(
      authenticatedPage.getByRole("button", { name: /Mã QR/i }).first(),
    ).toBeVisible();
  });

  test("QR image renders after entering step 5", async ({
    authenticatedPage,
  }) => {
  ...
  test("resolve_url is displayed in the URL box", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto(`/admin/items/${ITEM_ID}?step=5`);
  ...
  test("private item shows riêng tư warning banner", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.route(
      `**/api/v1/items/${ITEM_ID}`,
      (route: Route) => route.fulfill({ json: itemPrivateResponse }),
    );

    await authenticatedPage.goto(`/admin/items/${ITEM_ID}?step=5`);

    await expect(authenticatedPage.getByText(/chế độ riêng tư/i)).toBeVisible();
  });

  test("error state shown and retry button visible on QR API failure", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.route(
      `**/api/v1/items/${ITEM_ID}/qr`,
      (route: Route) =>
        route.fulfill({
          status: 500,
    ...
    await expect(
      authenticatedPage.getByText(/Không thể tải mã QR/i),
    ).toBeVisible();
    await expect(
      authenticatedPage.getByRole("button", { name: /Thử lại/i }),
    ).toBeVisible();
  });

  test("retry button fetches QR again and renders image on success", async ({
    authenticatedPage,
  }) => {
    let callCount = 0;
    await authenticatedPage.route(
    ...
    await expect(
      authenticatedPage.getByAltText("Mã QR sản phẩm"),
    ).toBeVisible();
    expect(callCount).toBe(2);
  });

  test("download link has correct filename attribute", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto(`/admin/items/${ITEM_ID}?step=5`);
  ...
  test("shows save-first message for new item without fetching QR", async ({
    authenticatedPage,
  }) => {
    let qrFetched = false;
    ...
    await expect(
      authenticatedPage.getByText(/Lưu sản phẩm trước khi tạo mã QR/i),
    ).toBeVisible();
  });
});
...
test.describe("Public — QR source banner", () => {
  ...
  test("shows QR banner when source=qr in URL", async ({ page }) => {
    ...
    await expect(
      page.getByText("Bạn đang xem sản phẩm qua mã QR"),
    ).toBeVisible();
  });
  ...
  test("QR banner and item name are both visible when source=qr", async ({
    page,
  }) => {
    await page.goto(`/items/${ITEM_ID}?shop_id=shop-a&source=qr`);
