...
describe('ItemDetailPage — QR step 5', () => {
  ...
  it('apiClient.get called with /items/1/qr when entering step 5', async () => {
    ...
    render(<ItemDetailPage />);
    await waitFor(() => {
      expect(h.apiClient.get).toHaveBeenCalledWith('/items/1/qr', expect.anything());
    });
  });
  ...
  it('resolve_url displayed in URL box', async () => {
    ...
    render(<ItemDetailPage />);
    await waitFor(() => {
      expect(screen.getByText('https://api.example.com/api/v1/public/qr/abc123def456789a')).toBeTruthy();
    });
  });
  ...
  it('Error state shown on API failure', async () => {
    ...
    render(<ItemDetailPage />);
    await waitFor(() => {
      expect(screen.getByText('Không thể tải mã QR. Vui lòng thử lại.')).toBeTruthy();
    });
  });
  ...
  it('Copy link button calls navigator.clipboard.writeText with resolve_url and shows success toast', async () => {
    ...
    await waitFor(() => {
      ...
      expect(h.mockShowToast).toHaveBeenCalledWith('Đã copy link QR', 'success');
    });
  });
