...
export class PreordersCrudService {
  ...
  async assertMemberInShop(memberId: string, shopId: string): Promise<void> {
  ...
  async assertActiveShop(shopId: string): Promise<void> {
  ...
  async assertPreorderAssigneeAllowed(
    shopId: string,
  ...
  async assertActorCanMutatePreorder(
    shopId: string,
  ...
  async findPublicCards(shopId: string, query: QueryPreordersDto) {
    ...
    const where: Prisma.PreOrderWhereInput = {
      shop_id: shopId,
      status: { in: [...PreordersCrudService.PUBLIC_STATUSES] },
      item: { is_public: true, deleted_at: null },
    };
    if (query.item_id) where.item_id = query.item_id;
    if (query.status) {
      if (!PreordersCrudService.PUBLIC_STATUSES.includes(query.status)) {
        throw new AppException(
          ErrorCode.VALIDATION_ERROR,
