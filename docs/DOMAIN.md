...
- Đóng/mở lại preorder: admin có thể đóng sớm cửa sổ preorder qua `PATCH /items/:id/close-preorder` (set `preorder_closes_at = NOW()`). Để mở lại, dùng `PATCH /items/:id/reopen-preorder` (xóa `preorder_closes_at`, reset `preorder_opens_at` về `created_at`, về open-ended). Các hành động này không thay đổi `status` item. Admin tự đặt deadline mới qua PATCH thông thường sau khi mở lại.
